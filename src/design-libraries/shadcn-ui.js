              ...
              <ul className="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                {section.items.map((item, itemIndex) => (<li key={itemIndex} className={`row-span-${item.rowSpan ?? 1}`}>
                    <NavigationMenuLink asChild>
                      <a className="hover:bg-accent hover:text-accent-foreground 
                        focus:bg-accent focus:text-accent-foreground block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors" href={item.href}>
                        {item.logo}
